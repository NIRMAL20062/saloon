...
**Security checklist:** role always read from `profiles`, never inferred client-side; dev-only login shortcut (mirrors the Expo `__DEV__`-gated one) compiled out via `BuildConfig.DEBUG`, never shippable.
